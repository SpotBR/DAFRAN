...
translate([larguraDaCorreia*1.14,-comprimentoDaCorreia,raio+espessuraDaCorreia/2])rotate([ 0, 90, 0])cylinder(profundidadeParaRolamento*3,raioDoRolamento,raioDoRolamento,$fn=100);
    ...
    rotate([ 0, 0, 45])translate([ 0, -176, 28 + comprimentoDaCorreia*6])
rotate([ -90, 0, 0])
translate([larguraDaCorreia*1.605,-comprimentoDaCorreia,raio+espessuraDaCorreia/2])rotate([ 0, 90, 0])encaixeRolamento(cima = true,baixo = false,recorteDoRecipiente=false);
}
if(recipienteFurado){
recipienteFurado();
}
...
if(colher){
    ...
    translate([-100,0,0])scale([(1 / 100) * escala, (1 / 100) * escala, (1 / 100) * escala])colher();
}
if(poliaDeCima){
