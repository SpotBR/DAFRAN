...
module dentes(correia,polia,correiaCurva){
    //DENTES
    ...
    if( (j * espacoDosDentes + fundoDoDente * j) < (comprimentoDaCorreia1-1))
    if(correiaCurva)
        translate([0, 0, -(j * espacoDosDentes + fundoDoDente * j)])
        dente(a=false,b=true);
}
}
module correiaComDentes(reta,curva) {    
    color("dimgray"){
        if(reta){
    difference() {
        minkowski() {
...
if(curva){
    difference() {
        minkowski() {
            translate([0, 0, -comprimentoDaCorreia1 / 2 ])cube([espessuraDaCorreia , larguraDaCorreia-arredondar*2,comprimentoDaCorreia1], true);
            sphere(arredondar, $fn = qualidade);
        }
        ...
        translate([0, 0,-(comprimentoDaCorreia1)-fundoDoDente ]) cube([2 * espessuraDaCorreia, larguraDaCorreia * 2, fundoDoDente*2], true);
        
        translate([0, 0, arredondar]) cube([2 * espessuraDaCorreia, larguraDaCorreia * 2, arredondar * 2], true);
        
        translate([0, 0,  - espacoEntreAsColheres]) rotate([0, atan((espessuraDaCorreia/2) / (comprimentoDaCorreia1)) + 90, 0]) translate([0, 0, 0.1]) recorte();
    }
    dentes(correia=false,polia=false,correiaCurva=true);
}
}
}
//dentes();
...
//correiaComDentes(reta=true,curva=false);